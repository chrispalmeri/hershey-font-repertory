...
let pages = await loadData('NWL Page');
let occidental = await loadData('NWL Occidental');
let oriental = await loadData('NWL Oriental');
...
pages.forEach(function(page) {
	page.Bearings = page.Bearings === '1' ? true : false;
	page.Nearby = page.Nearby === '1' ? true : false;
	page.Front = page.Front === '1' ? true : false;
	...
	let pageGlyphs = dataAccess[page.Data].filter(x => x.Page === page.Page);
...
function serializeRange(group, range) {
	...
	let covered = dataAccess[group].filter(x => parseInt(x['Glyph Number']) >= min && parseInt(x['Glyph Number']) <= max);
...
async function makeSvg(page) {
	let alldata = dataAccess[page.Data];

	// TODO: get rid of these
	let columns = parseInt(page.Columns);
	let rows = parseInt(page.Rows);
	let width = parseInt(page.Width);
	let height = parseInt(page.Height);
	let offset = parseInt(page['Vertical Offset']);
	let bearings = page.Bearings;
	let nearby = page.Nearby;
	let front = page.Front;

	// allow either new or old type of range
	let range = page.Range || parseRange(page.glyphs);
